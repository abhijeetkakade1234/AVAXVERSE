            ...
            <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                    ...
                    {!isConnected ? (
                        ...
                        <div className="flex items-center bg-white/30 dark:bg-black/20 rounded-full px-2 py-1 shadow-sm border border-white/40 dark:border-white/10">
                            ...
                            <button
                                className={`px-4 py-2 text-sm font-medium rounded-full transition-colors ${tab === 'manage' ? 'bg-white dark:bg-surface-dark shadow-sm' : 'hover:bg-white dark:hover:bg-surface-dark'}`}
                ...
                {!isConnected ? (
                    <div className="glass-panel bg-card-light dark:bg-card-dark border border-white/40 dark:border-white/10 rounded-3xl p-20 text-center">
                        ...
                        <div className="mt-4 flex items-center gap-2 text-xs text-text-muted-light dark:text-text-muted-dark bg-primary/5 p-3 rounded-xl border border-primary/10">
                                ...
                                <div className="glass-panel bg-card-light dark:bg-card-dark border border-white/40 dark:border-white/10 rounded-3xl p-4 mb-10 shadow-sm flex flex-col lg:flex-row items-center gap-4">
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                                        <div className="relative min-w-[160px] flex-1 lg:flex-initial flex items-center gap-2">
                                            ...
                                            <select
                                                className="w-full bg-white/30 dark:bg-black/20 border-none rounded-2xl py-3 pl-2 pr-8 focus:ring-2 focus:ring-primary text-sm font-bold appearance-none outline-none"
                                        ...
                                        <div className="relative min-w-[160px] flex-1 lg:flex-initial">
                                            <select
                                                className="w-full bg-white/30 dark:bg-black/20 border-none rounded-2xl py-3 pl-4 pr-10 focus:ring-2 focus:ring-primary text-sm font-bold appearance-none outline-none"
                                                value={rewardRange}
                                                onChange={e => setRewardRange(e.target.value)}
                                            >
                                                {REWARD_RANGES.map(r => <option key={r}>{r}</option>)}
                                            </select>
                                        </div>
                                    </div>
                                </div>

                                <BrowseJobs search={search} stateFilter={specialization} rewardFilter={rewardRange} />
                            </>
                        )}
